...
3. ヘッドホンを接続・切断したときなどの音声出力デバイスの切り替えが上手くいかず、再生が停止することがあります。
...
## 使い方
おことわり: 以下の操作手順は過去のバージョンをもとに記載したものであり、現在のバージョンでは異なる操作が必要な場合があります。

### 事前準備
[.NET 10.0 ランタイム](https://dotnet.microsoft.com/ja-jp/download)をインストールしてください。
ランタイムの選択を求められたら「.NET デスクトップ ランタイム」を選択してください。

Visual Studio 等を使用してソースからビルドした場合は、既にその PC に .NET SDK が入っているはずなので、ランタイムのインストールは不要です。
...
4. 画面下部の再生ボタンをクリックしトラックを再生すると、設定した通りにループ再生されます。
